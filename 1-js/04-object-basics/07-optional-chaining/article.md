...
1. `obj?.prop` -- returns `obj.prop` if `prop` exists and is not `null`, otherwise `undefined`.
2. `obj?.[prop]` -- returns `obj[prop]` if `prop` exists and is not `null`, otherwise `undefined`.
3. `obj.method?.()` -- calls `obj.method()` if `method` exists and is callable, otherwise returns `undefined`.
